...
def add_args(parser):
    ...
    parser.add_argument('--config_file', required=False,
                        default="configs/PURE_TYrPPG.yaml", type=str, help="The name of the model.")
    return parser


def train_and_test(config, data_loader_dict, train_data_loader):
    """Trains the model."""
    
    if config.MODEL.NAME == "TYrPPG":
        model_trainer = TYrPPGTrainer.TYrPPGTrainer(config, data_loader_dict, train_data_loader)
    elif config.MODEL.NAME == "Tscan":
        model_trainer = TscanTrainer.TscanTrainer(config, data_loader_dict)    
    elif config.MODEL.NAME == "Physnet":
        model_trainer = PhysnetTrainer.PhysnetTrainer(config, data_loader_dict, train_data_loader)
    elif config.MODEL.NAME == 'PhysFormer':
        model_trainer = PhysFormerTrainer.PhysFormerTrainer(config, data_loader_dict)    
    # if config.MODEL.NAME == 'DeepPhys':
        # model_trainer = DeepPhysTrainer.DeepPhysTrainer(config, data_loader_dict)        
    # elif config.MODEL.NAME == "Tscan":
...
def test(config, data_loader_dict,  train_data_loader):
    """Tests the model."""
    if config.MODEL.NAME == "TYrPPG":
        model_trainer = TYrPPGTrainer.TYrPPGTrainer(config, data_loader_dict, train_data_loader)
    elif config.MODEL.NAME == "Physnet":
...
if __name__ == "__main__":
    ...
    parser = BaseTrainer.BaseTrainer.add_trainer_args(parser)
    parser = BaseLoader.BaseLoader.add_data_loader_args(parser)
    ...
    if config.TOOLBOX_MODE == "train_and_test":
        ...
        if config.TRAIN.DATA.DATASET == "UBFC-PHYS":
            train_loader = UBFCPHYSLoader.UBFCPHYSLoader
        elif config.TRAIN.DATA.DATASET == "PURE":
            train_loader = PURELoader.PURELoader 
        elif config.TRAIN.DATA.DATASET == "MMPD":
            train_loader = MMPDLoader.MMPDLoader      
        else:
        ...
        if config.VALID.DATA.DATASET == "UBFC-PHYS":
            valid_loader = UBFCPHYSLoader.UBFCPHYSLoader
        elif config.VALID.DATA.DATASET == "PURE":
            valid_loader = PURELoader.PURELoader
        elif config.VALID.DATA.DATASET == "MMPD":
            valid_loader = MMPDLoader.MMPDLoader        
    ...
    if config.TOOLBOX_MODE == "train_and_test" or config.TOOLBOX_MODE == "only_test":
        ...
        if config.TEST.DATA.DATASET == "UBFC-PHYS":
            test_loader = UBFCPHYSLoader.UBFCPHYSLoader
        elif config.TEST.DATA.DATASET == "PURE":
            test_loader = PURELoader.PURELoader 
        elif config.TEST.DATA.DATASET == "MMPD":
            test_loader = MMPDLoader.MMPDLoader       
        else:
            raise ValueError("Unsupported dataset! Currently supporting UBFC-rPPG, PURE, MMPD, \
                             SCAMPS, BP4D+ (Normal and BigSmall preprocessing), and UBFC-PHYS.")
    ...
    if config.TOOLBOX_MODE == "train_and_test":
        train_and_test(config, data_loader_dict, train_data_loader)
    elif config.TOOLBOX_MODE == "only_test":
        test(config, data_loader_dict, test_loader)
    # elif config.TOOLBOX_MODE == "unsupervised_method":
