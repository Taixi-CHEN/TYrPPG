...
class TYrPPGTrainer(BaseTrainer):
    ...
    def __init__(self, config, data_loader, POSloader):
        super().__init__()
        ...
        if config.TOOLBOX_MODE == "train_and_test":
            self.model = TYrPPG().to(self.device)
        ...
        # print("Successfully uses pretrained model!")
    ...
    def train(self, data_loader):
        ...
        for epoch in range(self.max_epoch_num):
            ...
            for idx, batch in enumerate(tbar):
                ...
                # # 转换为 NumPy 数组
                # l1 = data.detach().cpu().numpy()  
                # POS_labels = torch.from_numpy(self.pos.generate_pos_psuedo_labels(l1)).to(self.device)  
            ...
            #         self.best_epoch = epoch
            #         print("Update best model! Best epoch: {}".format(self.best_epoch))
            test_loss = self.test(data_loader)
            print(test_loss)
    ...
    def test(self, data_loader):
        ...
        #     self.model_dir, self.model_file_name + '_Epoch' + str(self.current_epoch) + '.pth') # self.max_epoch_num - 1 self.current_epoch
        ...
        for i in range(0, 50):
            # last_epoch_model_path = os.path.join(
            # self.model_dir, self.model_file_name + '_Epoch' + str(i) + '.pth') # self.max_epoch_num - 1 self.current_epoch
            # print(last_epoch_model_path)
            # self.model.load_state_dict(torch.load(last_epoch_model_path))
            self.model.load_state_dict(torch.load(self.config.INFERENCE.MODEL_PATH))
            print("Testing uses pretrained model!")
